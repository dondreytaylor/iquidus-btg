...
const tx_cache = new NodeCache({ stdTTL: 100, checkperiod: 120 });
...
module.exports = {
  ...
  get_supply: function(cb) {
    if ( settings.supply == 'HEAVY' ) {
      ...
      console.log('Checking supply with gettxoutsetinfo.');
      Stats.findOne({coin: settings.coin}, function(err, stats) {
        return cb(stats.supply);
      });
    } else if (settings.supply == 'FORK_ESTIMATE') {
      var uri = base_url + 'getblockcount';
      request({uri: uri, json: true}, function(error, response, body) {
        return cb(((body - settings.fork_block) * settings.block_reward) + settings.fork_supply);
      });
    } else {
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].scriptPubKey.type === 'witness_v0_scripthash') {
        module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
          arr_vout.push({addresses: vout[i].scriptPubKey.type, scripthash: vout[i].scriptPubKey.hex, amount: amount_sat});
          loop.next();
        });
      } else if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') {
        if (('addresses' in vout[i].scriptPubKey)){
  ...
  get_input_addresses: function(input, vout, cb) {
    ...
    if (input.coinbase) {
      ...
      console.log('Getting input transaction: (%s)', input.txid);
      module.exports.get_rtx_cached(input.txid, function(tx){
        if (tx) {
          module.exports.syncLoop(tx.vout.length, function (loop) {
            ...
            if (tx.vout[i].n == input.vout) {
              if (tx.vout[i].scriptPubKey.addresses) {
                ...
                addresses.push({hash: tx.vout[i].scriptPubKey.hex, amount:tx.vout[i].value});
                loop.next();
              }
              loop.break(true);
              loop.next();
...
module.exports.last_tx = {txid: -1}
module.exports.get_rtx_cached= function (hash, cb){
  cached_tx = tx_cache.get(hash);
  if (cached_tx == undefined) {
    module.exports.get_rawtransaction(hash, function(tx){
      tx_cache.set(hash, tx);
      return cb(tx);
    });
  } else {
    return cb(cached_tx);
